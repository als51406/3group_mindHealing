        ...
        try {
            ...
            const getMessage = (e: unknown) => {
                if (e && typeof e === 'object' && 'message' in e) {
                    ...
                    return m ? String(m) : '네트워크 오류가 발생했습니다.';
                }
    ...
    return (
        <div style={{ width: '100%', minHeight: 'calc(100vh - 56px)', display: 'grid', placeItems: 'center', background: 'linear-gradient(180deg, #f9fafb 0%, #eef2ff 100%)' }}>
            <div style={{ width: 'min(420px, 92%)', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 20, boxShadow: '0 10px 30px rgba(0,0,0,0.06)' }}>
                <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 12 }}>로그인</div>
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                        <button type="submit" disabled={loading} style={{ padding: '10px 14px', borderRadius: 8, border: '1px solid #2563eb', background: loading ? '#93c5fd' : '#2563eb', color: '#fff', cursor: loading ? 'not-allowed' : 'pointer', flex: 1 }}>
                            {loading ? '로그인 중...' : '로그인'}
                        </button>
